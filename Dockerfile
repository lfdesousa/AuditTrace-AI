...
# ADR-047 (2026-06-19): embedding runs on the dedicated nomic-embed-server,
# NOT in this process. The old build step here baked ChromaDB's stock
# all-MiniLM-L6-v2 ONNX model (~79 MB) into the image to avoid a first-request
# download stall. With server-side embedding the gateway never loads an ONNX
# model, so that bake is removed — ~79 MB smaller image, no in-process model.
