...
3. **The Bratschi POC milestone schedule.** M1 (this ADR) is the
...
- Bratschi-tier production targets in-cluster Vault anyway (own
...
Per the Bratschi roadmap §"architecture documentation in lock-step"
...
- **Dynamic Postgres credentials.** Vault's database secret engine
  ...
  Strong story for Bratschi-tier ops; defer to a separate ADR.
