...
> **Current chat model (2026-08-30):** Qwen 3.8-27B (dense) + multi-token prediction (MTP). The Qwen 3.6-35B-A3B MoE was retired 2026-08-16. The captured request, response body, and latency figures below are preserved verbatim from earlier 35B-A3B runs and are a genuine reconstruction record, deliberately left unaltered. The current five-layer memory model adds a shared corpus to the four per-user layers shown (ADR-062).
...
|---|---|---|
| `user_id` | `<your-keycloak-sub>` | Keycloak `sub` claim on the JWT |
| `session_id` | `<session-id>` | `_compute_session_id(source, first_user, user_id)` — sha256 over the tuple |
...
## Cost & scale (orientational figures — not marketing claims)

Reference deployment: one HP ZBook with an integrated GPU, k3s single-node, Istio STRICT mTLS, all memory layers + Keycloak + three llama-server inference endpoints (Qwen 3.6-35B for chat/tools, nomic-embed v1.5 for embeddings, Mistral 7B Instruct for session summarisation). Chat completion latency distribution from a 100-probe evaluation (2026-04-17): mean 55.7 s, median 51.3 s, p95 120.6 s, 0 % error rate on tools-mode. Memory-server overhead excluding LLM inference: **< 1 ms per request** — every database write, Redis cache hit, MinIO read, and Langfuse ingestion call is microsecond-scoped (visible span-by-span in the Tempo flamegraph above).

Storage cost per request (order of magnitude): one `interactions` row (~5 KB uncompressed in Postgres), 0–4 `tool_calls` rows (~1 KB each), one Langfuse trace with ~20 observations (~30 KB), ~50 Tempo spans (~15 KB after compression), ~3 KB of Loki log lines. At 1 000 requests/day/user, ~150 MB/day/user of audit data. Linear scaling; no hidden multipliers.

Horizontal scale path: the reference Helm chart runs one memory-server replica per node; the FastAPI handler is stateless (ADR-027 moved episodic/procedural to MinIO). Postgres is the only shared-state component and runs a single writer with RLS; multi-region would use read replicas + logical replication, not a sharded rewrite. Reference architecture deliberately favours operational simplicity over maximum throughput — the target workload is regulated-industry audit, not Tier-1 exchange traffic.

Production-grade scaling, IdP federation (Google / Okta / Entra ID brokering), async job patterns for long generations, and per-tenant cost accounting are on the roadmap but not claimed as shipped. What's shipped is today's commit tree on `main` — tagged `v1.0.0`.
