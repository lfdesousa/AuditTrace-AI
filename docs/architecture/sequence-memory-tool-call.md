...
sequenceDiagram
    ...
    participant Registry as tools_visible_to<br/>+ get_tool_by_name
    participant Cache as ToolResultCache<br/>(sovereign-redis)
    participant Invoke as invoke_tool
    participant Episodic as EpisodicService
    participant LLM as llama-server :11435
    participant PG as PostgreSQL

    Agent->>Handler: POST /v1/chat/completions<br/>{messages: [{user: "KV cache?"}], project: "AuditTrace"}
    Handler->>Auth: Depends(require_user)
    Auth-->>Handler: UserContext (admin sentinel or Keycloak)

    Note over Handler: Build ambient context<br/>profile + project + date + tool hints
    Handler->>Registry: tools_visible_to(user)
    Registry-->>Handler: 4 memory tool defs (scope-filtered)

    Note over Handler: Merge ambient into system message<br/>augmented_tools = client_tools + memory_tools

    Handler->>Loop: run_memory_tool_loop(<br/>  llama_url, loop_payload,<br/>  user, session_id, max_iter=5)

    rect rgb(220, 230, 250)
        Note over Loop,LLM: Iteration 1 — non-streaming

        Loop->>LLM: POST /chat/completions (stream=false)<br/>{messages: [...ambient, user],<br/>tools: [bash?, recall_decisions, ...]}
        LLM-->>Loop: {tool_calls: [{id: c1, name: recall_decisions,<br/>arguments: '{"query":"KV cache"}'}]}

        Note over Loop: All tool_calls are memory tools<br/>→ dispatch, don't exit

        Loop->>Invoke: invoke_tool(user, recall_decisions_tool,<br/>{"query":"KV cache"}, session_id)
        ...
        Note over Loop: Append assistant tool_calls message<br/>+ tool_result message to conversation<br/>Record PendingToolCall (audit row)
    end

    rect rgb(230, 250, 220)
        Note over Loop,LLM: Iteration 2 — final answer

        Loop->>LLM: POST /chat/completions (stream=false)<br/>{messages: [...ambient, user,<br/>assistant tool_calls, tool_result]}
        LLM-->>Loop: {content: "Based on ADR-009: 75% reduction."}<br/>finish_reason: "stop"
    ...
    Handler->>PG: INSERT InteractionRecord<br/>(user_id, question, answer, session_id, ...)
    PG-->>Handler: interaction_id

    Handler->>PG: INSERT ToolCall<br/>(interaction_id FK, user_id, tool_name,<br/>granted_scope, duration_ms, result_summary)
    PG-->>Handler: ok

    Handler-->>Agent: 200 OK<br/>{choices: [{message: {content: "Based on ADR-009..."}}]}
```
...
sequenceDiagram
    ...
    Agent->>Handler: POST /v1/chat/completions<br/>{messages: [{user: "hello"}]}

    Note over Handler: Build ambient context (~50 words)<br/>no memory search fires

    Handler->>Loop: run_memory_tool_loop(...)
    Loop->>LLM: POST (stream=false)
    LLM-->>Loop: {content: "Hi!", finish_reason: "stop"}

    Note over Loop: no tool_calls → exit immediately<br/>(1 iteration, 0 pending audit rows)
...
sequenceDiagram
    ...
    Loop->>Invoke: invoke_tool(user, recall_decisions,<br/>{"query":"KV cache"}, session_id)
    Invoke->>Cache: get(sha256(session|tool|args))
    Cache-->>Invoke: {matches, total, truncated}

    Note over Invoke: HIT — skip handler<br/>skip audit row<br/>return (cached, was_cache_hit=True)

    Invoke-->>Loop: (cached_result, True)

    Note over Loop: was_cache_hit=True<br/>→ PendingToolCall NOT appended<br/>→ tool_result still sent to LLM
```
...
sequenceDiagram
    ...
    Note over Loop: tool_calls contains "bash"<br/>→ external → exit loop<br/>return body unchanged, pending=[]

    Loop-->>Agent: passthrough: bash tool_call
    Note over Agent: Agent runs bash locally,<br/>re-submits with tool_result<br/>next turn
```
...
sequenceDiagram
    ...
    Loop->>Logger: "memory tool-call loop reached<br/>max iterations (5)"
    Loop-->>Loop: return (last_body, pending)

    Note over Loop: Caller renders whatever text/tool_calls<br/>are in the body; cap-hit is not an error
```
...
sequenceDiagram
    ...
    alt stream=false
        ...
        Handler->>Synth: _synthesize_sse_from_body(<br/>  final_body, requested_model)
