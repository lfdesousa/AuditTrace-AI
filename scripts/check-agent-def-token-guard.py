...
Scans the known agent-def locations:
  * ``.claude/agents/*.md``                          (public; ``.claude/``
    is gitignored, so this is this repo's own untracked copy)
  * ``$AUDITTRACE_PRIVATE_DIR/sdlc/agents/**/*.md``   (private SDLC fleet
    defs, recursive — covers the ``opencode/`` variant too; default
    ``~/work/audittrace-private``)

Both locations are OUTSIDE what ``git`` tracks for this repo (``.claude/``
is gitignored; the private dir is a separate repo entirely), so this is
intentionally NOT a git-staged-diff pre-commit hook — that pattern (see
``scripts/check-no-private-content.sh``) cannot see either path, since
``git diff --cached`` never lists gitignored or foreign-repo files. Run
this manually, or wire it into an operator's own cross-repo automation.
...
def default_scan_targets(
    *, public_agents_dir: Path | None = None, private_dir: Path | None = None
) -> list[Path]:
    """The real agent-def locations this guard protects, best-effort.

    Both are optional-existence: a machine without the private repo (e.g. a
    fresh CI runner) simply contributes zero targets from that side rather
    than failing — this guard's job is to catch drift where the files exist,
    not to assert they must exist.
    """
    targets: list[Path] = []
    resolved_public = public_agents_dir or (
