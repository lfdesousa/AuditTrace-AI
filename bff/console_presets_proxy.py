"""The transparent, byte-faithful proxy to ``/console/presets/*``
(Mongo-repl WU-presets, MongoDB-elimination EPIC).

Mirrors ``bff/console_conversations_proxy.py``'s shape and discipline
exactly (see that module's docstring for the byte-fidelity rationale)
but targets the orchestrator's console-presets mount
(``settings.orchestrator_console_presets_path_prefix``) instead of
``/console/conversations`` — a genuinely different downstream mount, so
...
``/console/presets`` API returns for the exchanged token is relayed
...
class ConsolePresetsProxyError(Exception):
    """Raised when the orchestrator's ``/console/presets`` API cannot be
...
async def proxy_console_presets_request(
    method: str,
    path_suffix: str,
    query_string: str,
    raw_body: bytes,
    ...
    """Forward a ``/console/presets[/<path_suffix>]`` request to the
    orchestrator, byte-identical, and stream the response back
    byte-identical (including its status code).

    ``path_suffix`` may be empty (the base ``/console/presets`` path —
    list/create) — in that case no trailing slash is appended, so the
    orchestrator sees exactly ``/console/presets``, not
    ``/console/presets/``.
    """
    base = (
        settings.orchestrator_base_url.rstrip("/")
        + settings.orchestrator_console_presets_path_prefix
    )
    ...
    try:
        ...
        raise ConsolePresetsProxyError(
            f"orchestrator /console/presets unreachable: {exc}"
        ) from exc
