"""The transparent, byte-faithful proxy to ``/console/chat-projects/*``
...
targets the orchestrator's console-chat-projects mount
(``settings.orchestrator_console_chat_projects_path_prefix``) instead
of ``/console/presets`` — a genuinely different downstream mount, so
this is its own small module rather than a hardcoded prefix threaded
through an existing proxy function.

**Fail-closed on status code, always.** A 401/403/404 the orchestrator's
``/console/chat-projects`` API returns for the exchanged token is
...
class ConsoleChatProjectsProxyError(Exception):
    """Raised when the orchestrator's ``/console/chat-projects`` API
...
async def proxy_console_chat_projects_request(
    method: str,
    path_suffix: str,
    query_string: str,
    raw_body: bytes,
    ...
    """Forward a ``/console/chat-projects[/<path_suffix>]`` request to
    the orchestrator, byte-identical, and stream the response back
    byte-identical (including its status code).

    ``path_suffix`` may be empty (the base ``/console/chat-projects``
    path — list/create) — in that case no trailing slash is appended,
    so the orchestrator sees exactly ``/console/chat-projects``, not
    ``/console/chat-projects/``.
    """
    base = (
        settings.orchestrator_base_url.rstrip("/")
        + settings.orchestrator_console_chat_projects_path_prefix
    )
    ...
    try:
        ...
        raise ConsoleChatProjectsProxyError(
            f"orchestrator /console/chat-projects unreachable: {exc}"
        ) from exc
