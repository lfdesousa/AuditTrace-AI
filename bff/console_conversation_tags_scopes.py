...
``/console/conversation-tags/*`` proxy path (Conversation-Tags domain,
...
and ``bff/console_agents_scopes.py`` — same rationale as those modules'
...
rationale) because the downstream ``/console/conversation-tags/*``
surface is a full CRUD API with per-route scope enforcement already
happening at the orchestrator
(``src/audittrace/routes/console_conversation_tags.py`` — each route
declares its own ``Security(validate_jwt, scopes=[...])``); the BFF
only needs to mint a token broad enough to reach whichever specific
route the caller's request targets.
...
suite): ``tests/bff/test_console_conversation_tags_scopes.py`` fails the
moment a forbidden scope is added to
:data:`CONSOLE_CONVERSATION_TAGS_SCOPES`.
"""

from __future__ import annotations

CONSOLE_CONVERSATION_TAGS_SCOPES: tuple[str, ...] = (
...
# time rather than re-joining ``CONSOLE_CONVERSATION_TAGS_SCOPES`` per
# request.
CONSOLE_CONVERSATION_TAGS_SCOPE_STRING: str = " ".join(CONSOLE_CONVERSATION_TAGS_SCOPES)
