"""The transparent, byte-faithful proxy to ``/console/agents/*`` (Agents
...
but targets the orchestrator's console-agents mount
(``settings.orchestrator_console_agents_path_prefix``) instead of
``/console/chat-projects`` — a genuinely different downstream mount, so
...
``/console/agents`` API returns for the exchanged token is relayed
...
class ConsoleAgentsProxyError(Exception):
    """Raised when the orchestrator's ``/console/agents`` API cannot be
...
async def proxy_console_agents_request(
    method: str,
    path_suffix: str,
    query_string: str,
    raw_body: bytes,
    ...
    """Forward a ``/console/agents[/<path_suffix>]`` request to the
    orchestrator, byte-identical, and stream the response back
    byte-identical (including its status code).

    ``path_suffix`` may be empty (the base ``/console/agents`` path —
    list/create) — in that case no trailing slash is appended, so the
    orchestrator sees exactly ``/console/agents``, not
    ``/console/agents/``.
    """
    base = (
        settings.orchestrator_base_url.rstrip("/")
        + settings.orchestrator_console_agents_path_prefix
    )
    ...
    try:
        ...
        raise ConsoleAgentsProxyError(
            f"orchestrator /console/agents unreachable: {exc}"
        ) from exc
