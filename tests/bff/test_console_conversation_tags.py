"""End-to-end tests for ``/console/conversation-tags[/*]`` — the
console-conversation-tags proxy (Conversation-Tags domain,
...
Same MockTransport technique: one handler plays Keycloak (JWKS +
token-exchange) AND the orchestrator's ``/console/conversation-tags``
mount, routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  conversation-tags scopes, never the broad memory-proxy set or the
  ingest scope.
* ``TestPathForwarding`` — the base path (no suffix) and nested path
  (``{tag}``) reach the orchestrator at the correct URL, with the query
  string preserved for the list route.
...
from bff.console_conversation_tags_scopes import CONSOLE_CONVERSATION_TAGS_SCOPE_STRING
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_conversation_tags_path_prefix": (
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_CONVERSATION_TAGS_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsoleConversationTagsBackend:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/conversation-tags"):
...
@pytest.fixture
def console_conversation_tags_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsoleConversationTagsBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_conversation_tags_scope_pair(
        self, console_conversation_tags_client
    ) -> None:
        client, backend = console_conversation_tags_client
        token = make_token(sub="alice")
        client.get(
            "/console/conversation-tags", headers={"Authorization": f"Bearer {token}"}
        )
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_CONVERSATION_TAGS_SCOPE_STRING]
...
class TestPathForwarding:
    def test_base_path_no_suffix(self, console_conversation_tags_client) -> None:
        client, backend = console_conversation_tags_client
        token = make_token(sub="alice")
        client.get(
            "/console/conversation-tags", headers={"Authorization": f"Bearer {token}"}
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/conversation-tags"
        )
    ...
    def test_base_path_preserves_query_string(
        ...
        token = make_token(sub="alice")
        client.get(
            "/console/conversation-tags?limit=5&cursor=abc",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == (
            f"{ORCHESTRATOR_BASE}/console/conversation-tags?limit=5&cursor=abc"
        )

    def test_tag_path(self, console_conversation_tags_client) -> None:
        client, backend = console_conversation_tags_client
        token = make_token(sub="alice")
        client.get(
            "/console/conversation-tags/work",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/conversation-tags/work"
        )

    def test_delete_method_forwarded(self, console_conversation_tags_client) -> None:
        client, backend = console_conversation_tags_client
        token = make_token(sub="alice")
        client.delete(
            "/console/conversation-tags/work",
...
class TestFailClosed:
    def test_missing_authorization_header_401_never_reaches_orchestrator(
        ...
        client, backend = console_conversation_tags_client
        response = client.get("/console/conversation-tags")
    ...
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_conversation_tags_client, upstream_status: int
    ) -> None:
        client, backend = console_conversation_tags_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(
        ...
        client, backend = console_conversation_tags_client
        token = make_token(sub="alice")
        body = {"tag": "work", "description": "Hello"}
        client.post(
            "/console/conversation-tags",
            json=body,
            headers={"Authorization": f"Bearer {token}"},
