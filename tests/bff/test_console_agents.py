"""End-to-end tests for ``/console/agents[/*]`` — the console-agents
proxy (Agents domain, MongoDB-elimination EPIC).

Mirrors ``tests/bff/test_console_chat_projects.py``'s structure EXACTLY
(the spec's instruction), scaled to the agents field set. Same
MockTransport technique: one handler plays Keycloak (JWKS +
token-exchange) AND the orchestrator's ``/console/agents`` mount,
routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  agents scopes, never the broad memory-proxy set or the ingest scope.
* ``TestPathForwarding`` — the base path (no suffix) and nested paths
  (``{agent_id}``, ``batch-get``) reach the orchestrator at the correct
  URL, with the query string preserved for the list route.
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_agents_path_prefix": "/console/agents",
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_AGENTS_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsoleAgentsBackend:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/agents"):
...
@pytest.fixture
def console_agents_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsoleAgentsBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_agents_scope_pair(
        self, console_agents_client
    ) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.get("/console/agents", headers={"Authorization": f"Bearer {token}"})
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_AGENTS_SCOPE_STRING]
        assert parsed["scope"] != [MEMORY_SCOPE_STRING]

    def test_scope_pair_has_no_admin_no_corpus(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.get("/console/agents", headers={"Authorization": f"Bearer {token}"})
        parsed = parse_qs(backend.exchange_request_bodies[0])
...
class TestPathForwarding:
    def test_base_path_no_suffix(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.get("/console/agents", headers={"Authorization": f"Bearer {token}"})
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/agents"
        )

    def test_base_path_preserves_query_string(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.get(
            "/console/agents?limit=5&cursor=abc",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == (f"{ORCHESTRATOR_BASE}/console/agents?limit=5&cursor=abc")

    def test_agent_id_path(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.get(
        ...
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/agents/agent-1"
        )

    def test_batch_get_path(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.post(
        ...
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/agents/batch-get"
        )

    def test_delete_method_forwarded(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        client.delete(
            "/console/agents/agent-1",
...
class TestIdentityPropagation:
    def test_two_callers_produce_two_distinct_minted_subs(
        ...
        client.get(
            "/console/agents", headers={"Authorization": f"Bearer {alice_token}"}
        )
        client.get("/console/agents", headers={"Authorization": f"Bearer {bob_token}"})
...
class TestFailClosed:
    ...
    def test_expired_token_401_never_reaches_orchestrator(
        ...
        response = client.get(
            "/console/agents", headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 401
        assert backend.orchestrator_calls == []
    ...
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_agents_client, upstream_status: int
    ) -> None:
        client, backend = console_agents_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(self, console_agents_client) -> None:
        client, backend = console_agents_client
        token = make_token(sub="alice")
        body = {"agent_id": "agent-1", "name": "Hello"}
        client.post(
            "/console/agents",
            json=body,
            headers={"Authorization": f"Bearer {token}"},
