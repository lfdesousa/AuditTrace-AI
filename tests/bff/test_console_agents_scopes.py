"""Tests for bff/console_agents_scopes.py — the console-agents scope
pair (Agents domain, MongoDB-elimination EPIC).
...
proof that this exchange can never widen beyond the two agents scopes.
"""

from __future__ import annotations

from bff.console_agents_scopes import (
    CONSOLE_AGENTS_SCOPE_STRING,
    CONSOLE_AGENTS_SCOPES,
)


class TestConsoleAgentsScopesContent:
    def test_exact_expected_scope_set(self) -> None:
        assert set(CONSOLE_AGENTS_SCOPES) == {
            "memory:agents:read-own",
            "memory:agents:write",
        }

    def test_no_duplicate_scopes(self) -> None:
        assert len(CONSOLE_AGENTS_SCOPES) == len(set(CONSOLE_AGENTS_SCOPES))

    def test_exactly_two_scopes(self) -> None:
        assert len(CONSOLE_AGENTS_SCOPES) == 2


class TestNeverForbiddenScope:
    """Falsifiable: add ``audittrace:admin``, any ``memory:corpus:*``, or
    any other layer's read/write scope to :data:`CONSOLE_AGENTS_SCOPES`
    and these tests go RED."""

    def test_admin_scope_absent(self) -> None:
        assert "audittrace:admin" not in CONSOLE_AGENTS_SCOPES

    def test_admin_scope_absent_from_scope_string(self) -> None:
        assert "audittrace:admin" not in CONSOLE_AGENTS_SCOPE_STRING.split(" ")

    def test_no_corpus_scope_present(self) -> None:
        assert not any(s.startswith("memory:corpus:") for s in CONSOLE_AGENTS_SCOPES)
    ...
    def test_no_other_layer_scope_present(self) -> None:
        ...
        assert other_layer_scopes.isdisjoint(CONSOLE_AGENTS_SCOPES)
    ...
    def test_scope_string_never_shares_other_proxy_scope_strings(self) -> None:
        ...
        assert CONSOLE_AGENTS_SCOPE_STRING != MEMORY_SCOPE_STRING
        assert CONSOLE_AGENTS_SCOPE_STRING != INGEST_SCOPE_STRING
        assert CONSOLE_AGENTS_SCOPE_STRING != CONSOLE_CONVERSATIONS_SCOPE_STRING
        assert CONSOLE_AGENTS_SCOPE_STRING != CONSOLE_PRESETS_SCOPE_STRING
        assert CONSOLE_AGENTS_SCOPE_STRING != CONSOLE_PROMPTS_SCOPE_STRING
        assert CONSOLE_AGENTS_SCOPE_STRING != CONSOLE_CHAT_PROJECTS_SCOPE_STRING
        assert CONSOLE_AGENTS_SCOPE_STRING != CONSOLE_FILE_RECORDS_SCOPE_STRING
        assert set(CONSOLE_AGENTS_SCOPES).isdisjoint(
        ...
        assert set(CONSOLE_AGENTS_SCOPES).isdisjoint(
            set(CONSOLE_FILE_RECORDS_SCOPE_STRING.split(" "))
        )


class TestConsoleAgentsScopeString:
    def test_exact_value(self) -> None:
        assert (
            CONSOLE_AGENTS_SCOPE_STRING == "memory:agents:read-own memory:agents:write"
        )

    def test_space_separated_matches_rfc8693_scope_shape(self) -> None:
        assert CONSOLE_AGENTS_SCOPE_STRING == " ".join(CONSOLE_AGENTS_SCOPES)
        assert CONSOLE_AGENTS_SCOPE_STRING.split(" ") == list(CONSOLE_AGENTS_SCOPES)
