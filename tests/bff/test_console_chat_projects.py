"""End-to-end tests for ``/console/chat-projects[/*]`` — the
console-chat-projects proxy (Chat-Projects domain, MongoDB-elimination
EPIC).

Mirrors ``tests/bff/test_console_presets.py``'s structure EXACTLY (the
spec's instruction), scaled to the chat-projects field set. Same
MockTransport technique: one handler plays Keycloak (JWKS +
token-exchange) AND the orchestrator's ``/console/chat-projects`` mount,
routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  chat-projects scopes, never the broad memory-proxy set or the ingest
  scope.
* ``TestPathForwarding`` — the base path (no suffix) and nested path
  (``{chat_project_id}``) reach the orchestrator at the correct URL,
  with the query string preserved for the list route.
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_chat_projects_path_prefix": "/console/chat-projects",
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_CHAT_PROJECTS_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsoleChatProjectsBackend:
    def __init__(self) -> None:
        ...
        self.orchestrator_response = httpx.Response(
            200,
            json={"chat_project_id": "project-1"},
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/chat-projects"):
...
@pytest.fixture
def console_chat_projects_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsoleChatProjectsBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_chat_projects_scope_pair(
        self, console_chat_projects_client
    ) -> None:
        client, backend = console_chat_projects_client
        token = make_token(sub="alice")
        client.get(
            "/console/chat-projects", headers={"Authorization": f"Bearer {token}"}
        )
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_CHAT_PROJECTS_SCOPE_STRING]
...
class TestPathForwarding:
    def test_base_path_no_suffix(self, console_chat_projects_client) -> None:
        client, backend = console_chat_projects_client
        token = make_token(sub="alice")
        client.get(
            "/console/chat-projects", headers={"Authorization": f"Bearer {token}"}
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/chat-projects"
        )
    ...
    def test_base_path_preserves_query_string(
        ...
        token = make_token(sub="alice")
        client.get(
            "/console/chat-projects?limit=5&cursor=abc",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == (
            f"{ORCHESTRATOR_BASE}/console/chat-projects?limit=5&cursor=abc"
        )

    def test_chat_project_id_path(self, console_chat_projects_client) -> None:
        client, backend = console_chat_projects_client
        token = make_token(sub="alice")
        client.get(
            "/console/chat-projects/project-1",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/chat-projects/project-1"
        )

    def test_delete_method_forwarded(self, console_chat_projects_client) -> None:
        client, backend = console_chat_projects_client
        token = make_token(sub="alice")
        client.delete(
            "/console/chat-projects/project-1",
...
class TestIdentityPropagation:
    def test_two_callers_produce_two_distinct_minted_subs(
        ...
        client.get(
            "/console/chat-projects", headers={"Authorization": f"Bearer {bob_token}"}
        )
...
class TestFailClosed:
    def test_missing_authorization_header_401_never_reaches_orchestrator(
        ...
        client, backend = console_chat_projects_client
        response = client.get("/console/chat-projects")
    ...
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_chat_projects_client, upstream_status: int
    ) -> None:
        client, backend = console_chat_projects_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(
        ...
        client, backend = console_chat_projects_client
        token = make_token(sub="alice")
        body = {"chat_project_id": "project-1", "name": "Hello"}
        client.post(
            "/console/chat-projects",
            json=body,
            headers={"Authorization": f"Bearer {token}"},
    ...
    def test_orchestrator_response_body_returned_unchanged(
        ...
        assert response.json() == {"chat_project_id": "project-1"}
