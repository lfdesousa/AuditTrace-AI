"""End-to-end tests for ``/console/prompts[/*]`` — the console-prompts
proxy (Mongo-repl WU-prompts, MongoDB-elimination EPIC).

Mirrors ``tests/bff/test_console_presets.py``'s structure (the spec's
instruction), scaled to the group+versions nested-path shape. Same
MockTransport technique: one handler plays Keycloak (JWKS +
token-exchange) AND the orchestrator's ``/console/prompts`` mount,
routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  prompts scopes, never the broad memory-proxy set or the ingest scope.
* ``TestPathForwarding`` — the base path (no suffix) and nested paths
  (``{group_id}``, ``{group_id}/versions``, ``{group_id}/production``)
  reach the orchestrator at the correct URL, with the query string
  preserved for the list route and PATCH forwarded for set-production.
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_prompts_path_prefix": "/console/prompts",
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_PROMPTS_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsolePromptsBackend:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/prompts"):
...
@pytest.fixture
def console_prompts_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsolePromptsBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_prompts_scope_pair(
        self, console_prompts_client
    ) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.get("/console/prompts", headers={"Authorization": f"Bearer {token}"})
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_PROMPTS_SCOPE_STRING]
        assert parsed["scope"] != [MEMORY_SCOPE_STRING]

    def test_scope_pair_has_no_admin_no_corpus(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.get("/console/prompts", headers={"Authorization": f"Bearer {token}"})
        parsed = parse_qs(backend.exchange_request_bodies[0])
...
class TestPathForwarding:
    def test_base_path_no_suffix(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.get("/console/prompts", headers={"Authorization": f"Bearer {token}"})
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/prompts"
        )

    def test_base_path_preserves_query_string(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.get(
            "/console/prompts?limit=5&cursor=abc",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == (f"{ORCHESTRATOR_BASE}/console/prompts?limit=5&cursor=abc")

    def test_group_id_path(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.get(
            "/console/prompts/group-1",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/prompts/group-1"
        )

    def test_versions_path(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.post(
            "/console/prompts/group-1/versions",
            json={"prompt_id": "p1", "text": "hi"},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/prompts/group-1/versions"
        )

    def test_production_path_patch_forwarded(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.patch(
        ...
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/prompts/group-1/production"
        )
        assert backend.orchestrator_calls[0].method == "PATCH"

    def test_delete_method_forwarded(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        client.delete(
            "/console/prompts/group-1",
...
class TestIdentityPropagation:
    def test_two_callers_produce_two_distinct_minted_subs(
        ...
        client.get(
            "/console/prompts", headers={"Authorization": f"Bearer {alice_token}"}
        )
        client.get("/console/prompts", headers={"Authorization": f"Bearer {bob_token}"})
...
class TestFailClosed:
    def test_missing_authorization_header_401_never_reaches_orchestrator(
        ...
        client, backend = console_prompts_client
        response = client.get("/console/prompts")
    ...
    def test_expired_token_401_never_reaches_orchestrator(
        ...
        response = client.get(
            "/console/prompts", headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 401
        assert backend.orchestrator_calls == []
    ...
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_prompts_client, upstream_status: int
    ) -> None:
        client, backend = console_prompts_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(self, console_prompts_client) -> None:
        client, backend = console_prompts_client
        token = make_token(sub="alice")
        body = {"group_id": "group-1", "name": "Hello"}
        client.post(
            "/console/prompts",
            json=body,
            headers={"Authorization": f"Bearer {token}"},
    ...
    def test_orchestrator_response_body_returned_unchanged(
        ...
        assert response.json() == {"group_id": "group-1"}
