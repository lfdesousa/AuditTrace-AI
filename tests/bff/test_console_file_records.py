"""End-to-end tests for ``/console/file-records[/*]`` — the
console-files-metadata proxy (Files-metadata domain, MongoDB-elimination
EPIC).

Mirrors ``tests/bff/test_console_chat_projects.py``'s structure EXACTLY
(the spec's instruction), scaled to the files field set and the
BFF-facing path rename (``/console/file-records`` on the BFF, forwarding
to the orchestrator's real ``/console/files`` mount — see
``bff/config.py``'s ``orchestrator_console_files_path_prefix`` docstring
for why). Same MockTransport technique: one handler plays Keycloak
(JWKS + token-exchange) AND the orchestrator's ``/console/files`` mount,
routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  files scopes, never the broad memory-proxy set, the ingest scope, or
  any other domain's scope pair.
* ``TestPathForwarding`` — the BFF's ``/console/file-records`` base path
  (no suffix) and nested paths (``{file_id}``, ``batch-get``) reach the
  orchestrator at ``/console/files[...]`` — the RENAMED prefix, not a
  literal copy — with the query string preserved for the list route.
...
* ``TestIngestRouteUntouched`` — the pre-existing ``POST /console/files``
  ephemeral-ingest route (and its ``/promote`` sibling) keep behaving
  exactly as before; this new proxy never shadows or reroutes them.
"""
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_files_path_prefix": "/console/files",
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_FILE_RECORDS_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsoleFileRecordsBackend:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/files"):
...
@pytest.fixture
def console_file_records_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsoleFileRecordsBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_files_scope_pair(
        self, console_file_records_client
    ) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        client.get(
            "/console/file-records", headers={"Authorization": f"Bearer {token}"}
        )
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_FILE_RECORDS_SCOPE_STRING]
        assert parsed["scope"] != [MEMORY_SCOPE_STRING]
        assert parsed["scope"] != [INGEST_SCOPE_STRING]

    def test_scope_pair_has_no_admin_no_corpus_no_ingest(
        self, console_file_records_client
    ) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
...
class TestPathForwarding:
    def test_base_path_no_suffix_maps_to_orchestrator_console_files(
        self, console_file_records_client
    ) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/files"
        )

    def test_base_path_preserves_query_string(
        self, console_file_records_client
    ) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        client.get(
            "/console/file-records?limit=5&cursor=abc",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == (f"{ORCHESTRATOR_BASE}/console/files?limit=5&cursor=abc")

    def test_file_id_path(self, console_file_records_client) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        client.get(
            "/console/file-records/file-1",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/files/file-1"
        )

    def test_batch_get_path(self, console_file_records_client) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        client.post(
            "/console/file-records/batch-get",
            json={"file_ids": ["file-1", "file-2"]},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/files/batch-get"
        )
        assert backend.orchestrator_calls[0].method == "POST"

    def test_delete_method_forwarded(self, console_file_records_client) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        client.delete(
            "/console/file-records/file-1",
...
class TestIdentityPropagation:
    def test_two_callers_produce_two_distinct_minted_subs(
        ...
        client.get(
            "/console/file-records", headers={"Authorization": f"Bearer {bob_token}"}
        )
...
class TestFailClosed:
    def test_missing_authorization_header_401_never_reaches_orchestrator(
        ...
        client, backend = console_file_records_client
        response = client.get("/console/file-records")
    ...
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_file_records_client, upstream_status: int
    ) -> None:
        client, backend = console_file_records_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(
        self, console_file_records_client
    ) -> None:
        client, backend = console_file_records_client
        token = make_token(sub="alice")
        body = {"file_id": "file-1", "filename": "a.txt", "type": "text/plain"}
        client.post(
            "/console/file-records",
            json=body,
            headers={"Authorization": f"Bearer {token}"},
...
class TestIngestRouteUntouched:
    """The pre-existing ``POST /console/files`` ephemeral-ingest route
    (M3 Sovereign-Attach WU-2) must keep behaving exactly as before —
    this new ``/console/file-records`` proxy is a SEPARATE path and
    never shadows it. Falsifiable: mounting the new proxy at
    ``/console/files`` instead of ``/console/file-records`` would make
    this test fail (the ingest route would stop reaching
    ``/memory/upload`` and hit ``/console/files`` on the orchestrator
    instead)."""

    def test_ingest_route_still_forwards_to_memory_upload(self) -> None:
        from fastapi.testclient import TestClient
        ...
        assert response.status_code == 200
        assert any(u.startswith(f"{ORCHESTRATOR_BASE}/memory/upload") for u in calls)
        assert not any(
            u.startswith(f"{ORCHESTRATOR_BASE}/console/files") for u in calls
        )
