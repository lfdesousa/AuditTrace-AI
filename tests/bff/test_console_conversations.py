"""End-to-end tests for ``/console/conversations[/*]`` — the console-
conversations proxy (WU-1, MongoDB-elimination EPIC).

Same MockTransport technique as ``tests/bff/test_console_files.py``: one
handler plays Keycloak (JWKS + token-exchange) AND the orchestrator's
``/console/conversations`` mount, routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  conversations scopes, never the broad memory-proxy set or the ingest
  scope.
* ``TestPathForwarding`` — the base path (no suffix) and nested paths
  (``{conversation_id}``, ``{conversation_id}/messages``,
  ``{conversation_id}/messages/{message_id}``) reach the orchestrator at
  the correct URL, with the query string preserved for the list route.
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_conversations_path_prefix": "/console/conversations",
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_CONVERSATIONS_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsoleConversationsBackend:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/conversations"):
...
@pytest.fixture
def console_conversations_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsoleConversationsBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_conversations_scope_pair(
        self, console_conversations_client
    ) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        client.get(
            "/console/conversations", headers={"Authorization": f"Bearer {token}"}
        )
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_CONVERSATIONS_SCOPE_STRING]
        assert parsed["scope"] != [MEMORY_SCOPE_STRING]

    def test_scope_pair_has_no_admin_no_corpus(
        self, console_conversations_client
    ) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
...
class TestPathForwarding:
    def test_base_path_no_suffix(self, console_conversations_client) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        client.get(
            "/console/conversations", headers={"Authorization": f"Bearer {token}"}
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/conversations"
        )
    ...
    def test_base_path_preserves_query_string(
        ...
        token = make_token(sub="alice")
        client.get(
            "/console/conversations?limit=5&cursor=abc",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == (
            f"{ORCHESTRATOR_BASE}/console/conversations?limit=5&cursor=abc"
        )

    def test_conversation_id_path(self, console_conversations_client) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        client.get(
            "/console/conversations/conv-1",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/conversations/conv-1"
        )

    def test_messages_path(self, console_conversations_client) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        client.get(
        ...
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/conversations/conv-1/messages"
        )

    def test_message_id_path(self, console_conversations_client) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        client.patch(
        ...
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/conversations/conv-1/messages/msg-1"
        )
        assert backend.orchestrator_calls[0].method == "PATCH"

    def test_delete_method_forwarded(self, console_conversations_client) -> None:
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        client.delete(
            "/console/conversations/conv-1",
...
class TestIdentityPropagation:
    def test_two_callers_produce_two_distinct_minted_subs(
        ...
        client.get(
            "/console/conversations", headers={"Authorization": f"Bearer {alice_token}"}
        )
        client.get(
            "/console/conversations", headers={"Authorization": f"Bearer {bob_token}"}
        )
...
class TestFailClosed:
    ...
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_conversations_client, upstream_status: int
    ) -> None:
        client, backend = console_conversations_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(
        ...
        client, backend = console_conversations_client
        token = make_token(sub="alice")
        body = {"conversation_id": "conv-1", "title": "Hello"}
        client.post(
