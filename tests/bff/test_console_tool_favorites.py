"""End-to-end tests for ``/console/tool-favorites[/*]`` — the
console-tool-favorites proxy (Tool-Favorites domain, MongoDB-
elimination EPIC).

Mirrors ``tests/bff/test_console_conversation_tags.py``'s structure
EXACTLY (the spec's instruction), scaled to the tool-favorites field
set. Same MockTransport technique: one handler plays Keycloak (JWKS +
token-exchange) AND the orchestrator's ``/console/tool-favorites``
mount, routed by URL.

Guard classes:

* ``TestExactScopePair`` — the exchange sends ``scope`` EXACTLY the two
  tool-favorites scopes, never the broad memory-proxy set or the
  ingest scope.
* ``TestPathForwarding`` — the base path (no suffix) and nested path
  (``{item_type}/{item_id}``) reach the orchestrator at the correct
  URL.
...
* ``TestFailClosed`` — absent/invalid token never reaches Keycloak or the
  orchestrator; Keycloak failure is 502; the orchestrator's own 4xx
  (including the 409 cap-exceeded response) is relayed byte-for-byte.
...
def _settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        ...
        "orchestrator_console_tool_favorites_path_prefix": ("/console/tool-favorites"),
    ...
    return jwt.encode(
        {
            ...
            "scope": CONSOLE_TOOL_FAVORITES_SCOPE_STRING,
            "iat": now,
            "exp": now + 300,
        },
        TEST_PRIVATE_PEM,
...
class _ConsoleToolFavoritesBackend:
    def __init__(self) -> None:
        ...
        self.orchestrator_response = httpx.Response(
            200,
            json={"item_type": "tool", "item_id": "web-search"},
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if url.startswith(f"{ORCHESTRATOR_BASE}/console/tool-favorites"):
...
@pytest.fixture
def console_tool_favorites_client():
    from fastapi.testclient import TestClient

    app = create_app()
    backend = _ConsoleToolFavoritesBackend()
...
class TestExactScopePair:
    def test_exchange_requests_exactly_tool_favorites_scope_pair(
        self, console_tool_favorites_client
    ) -> None:
        client, backend = console_tool_favorites_client
        token = make_token(sub="alice")
        client.get(
            "/console/tool-favorites", headers={"Authorization": f"Bearer {token}"}
        )
        assert len(backend.exchange_request_bodies) == 1
        parsed = parse_qs(backend.exchange_request_bodies[0])
        assert parsed["scope"] == [CONSOLE_TOOL_FAVORITES_SCOPE_STRING]
...
class TestPathForwarding:
    def test_base_path_no_suffix(self, console_tool_favorites_client) -> None:
        client, backend = console_tool_favorites_client
        token = make_token(sub="alice")
        client.get(
            "/console/tool-favorites", headers={"Authorization": f"Bearer {token}"}
        ...
        assert (
            ...
            == f"{ORCHESTRATOR_BASE}/console/tool-favorites"
        )
    ...
    def test_base_path_preserves_query_string(
        ...
        token = make_token(sub="alice")
        client.get(
            "/console/tool-favorites?limit=5",
            headers={"Authorization": f"Bearer {token}"},
        )
        forwarded = str(backend.orchestrator_calls[0].url)
        assert forwarded == f"{ORCHESTRATOR_BASE}/console/tool-favorites?limit=5"

    def test_item_path(self, console_tool_favorites_client) -> None:
        client, backend = console_tool_favorites_client
        token = make_token(sub="alice")
        client.get(
            "/console/tool-favorites/tool/web-search",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert (
            str(backend.orchestrator_calls[0].url)
            == f"{ORCHESTRATOR_BASE}/console/tool-favorites/tool/web-search"
        )

    def test_delete_method_forwarded(self, console_tool_favorites_client) -> None:
        client, backend = console_tool_favorites_client
        token = make_token(sub="alice")
        client.delete(
            "/console/tool-favorites/tool/web-search",
...
class TestFailClosed:
    def test_missing_authorization_header_401_never_reaches_orchestrator(
        ...
        client, backend = console_tool_favorites_client
        response = client.get("/console/tool-favorites")
    ...
    @pytest.mark.parametrize("upstream_status", [400, 403, 404, 409])
    def test_orchestrator_denial_forwarded_verbatim_not_manufactured(
        self, console_tool_favorites_client, upstream_status: int
    ) -> None:
        client, backend = console_tool_favorites_client
...
class TestByteFaithfulForward:
    def test_json_body_forwarded_byte_identical(
        ...
        client, backend = console_tool_favorites_client
        token = make_token(sub="alice")
        body = {"item_type": "tool", "item_id": "web-search"}
        client.post(
            "/console/tool-favorites",
            json=body,
            headers={"Authorization": f"Bearer {token}"},
    ...
    def test_orchestrator_response_body_returned_unchanged(
        ...
        assert response.json() == {"item_type": "tool", "item_id": "web-search"}
