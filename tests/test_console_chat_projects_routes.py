...
class TestCrossUserIsolation:
    ...
    def test_hostile_body_user_sub_cannot_hijack_another_users_row(
        ...
        ``TestConsoleChatProjectsCrud::test_hostile_body_extra_fields_are_dropped_from_response``
        ...
        ``ConsoleChatProjectUpsertRequest`` AND the route honored it
        ...
        victim_create = self._act_as(
            ...
            "/console/chat-projects",
            json={
                "chat_project_id": "shared-hostile-id",
                "name": "victim's real chat-project",
            },
        )
        assert victim_create.status_code == 200

        # The attacker upserts the SAME chat_project_id, with a hostile
        ...
        victim_read = self._act_as(
            ...
            "victim-hostile",
            "GET",
            "/console/chat-projects/shared-hostile-id",
        )
        assert victim_read.status_code == 200
        assert victim_read.json()["name"] == "victim's real chat-project", (
            "attacker's hostile user_sub/user_id body fields hijacked/"
            "overwrote the victim's chat-project — the fields must be "
        ...
        # same chat_project_id), not the victim's.
        ...
        # chat_project_id, must never be visible to the victim via
        ...
        planted = self._act_as(
            ...
            "/console/chat-projects",
            json={
                "chat_project_id": "planted-hostile-id",
                "name": "planted",
                "user_sub": "victim-hostile",
                "user_id": "victim-hostile",
            },
        ...
        victim_get_planted = self._act_as(
            ...
            "victim-hostile",
            "GET",
            "/console/chat-projects/planted-hostile-id",
        )
        assert victim_get_planted.status_code == 404, (
            "the victim can read a chat-project planted via a hostile "
            "user_sub body field — the field must never be honored"
        )

        victim_list = self._act_as(
            client, "victim-hostile", "GET", "/console/chat-projects"
        )
        victim_ids = [i["chat_project_id"] for i in victim_list.json()["items"]]
        assert "planted-hostile-id" not in victim_ids, (
            "the victim's list included a chat-project planted via a "
