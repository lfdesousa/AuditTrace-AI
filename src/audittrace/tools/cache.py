...
key prefixes so they cannot collide:
  ...
  - ``audittrace:tool-result:<cache_id>``  — ToolResultCache (this file)
...
class ToolResultCache:
    ...
    Cache key shape: ``audittrace:tool-result:<cache_id>`` where ``cache_id``
    ...
    KEY_PREFIX = "audittrace:tool-result:"
    ...
    def clear(self) -> None:
        """Drop every cache entry under the ``audittrace:tool-result:`` prefix.
