...
ToolHandler = Callable[["UserContext", dict], Awaitable[dict]]
...
def tools_visible_to(user_context: UserContext) -> list[dict[str, Any]]:
    ...
    - Admins see every enabled tool regardless of ``required_scope``
      ...
      ``SOVEREIGN_AUTH_REQUIRED=false`` bypass keep working).
...
async def invoke_tool(
    ...
    from sovereign_memory.tools.cache import get_tool_result_cache
