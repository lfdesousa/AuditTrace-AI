...
| 🟡 In progress | **ADR-048 Content-Control** | External-PDF parser-exploit gap closed via a sibling pod (`audittrace-content-control`, separate identity, separate MinIO IAM role). PR-B1 (contract scaffolding) + PR-B2 (Vault / bucket-init / quarantine denylist) + PR-B2.5 (RabbitMQ subchart + AMQP topology + ADR-057 broker decision) + PR-B3 (`/memory/upload` PDF rewrite to 202 + Hohpe outbox + asyncio.Queue producer + janitor) merged. Repo-A side complete: PR-A1 skeleton + PR-A2 ClamAV adapter + PR-A3 async pipeline. PR-B4 verdict consumer + PR-A4 operator surfaces + PR-B7 MinIO IAM split remain before flip to Accepted. Gates M3 LibreChat Day-1 + Day-2 fork. |
