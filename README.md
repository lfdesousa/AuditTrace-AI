...
- ✅ **Comprehensive Test Suite** -- 3070 tests, 98% coverage, 90% per-file gate enforced in CI ([latest run](https://github.com/lfdesousa/AuditTrace-AI/actions/workflows/ci.yml)).
...
# Run tests (90% per-file coverage enforced; full suite is 3070 tests / 98% total)
make test
...
# In scripts / ad-hoc curl calls (--show-unsafe: this needs the REAL Bearer
# value; plain --show prints a redacted fingerprint instead):
BEARER=$(scripts/audittrace-login --show-unsafe)   # auto-refreshes if near expiry
...
- [ADR-048: Ingestion content control](docs/ADR-048-ingestion-content-control.md) *(Accepted)*
